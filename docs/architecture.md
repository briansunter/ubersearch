...
- **Config resolution**: `src/config/load.ts` searches explicit path → local `ai-search.config.(ts|json)` → XDG config. TypeScript configs use `defineConfig` helpers; JSON validated against `config.schema.json`.
...
- **Orchestration**: `AiSearchOrchestrator` executes strategies from `src/core/strategy/*`, collects attempts, and returns normalized results with credit snapshots.
...
- Credit state defaults to `~/.local/state/ai-search/credits.json`; override via config or CLI options that accept `creditStatePath` through bootstrap options.
